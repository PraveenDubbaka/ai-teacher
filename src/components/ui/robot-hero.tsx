...
function AntennaNavbar({
  ...
  return (
    <nav className="sticky top-0 z-50 w-full px-8 pt-8 pointer-events-none">
      <div className="mx-auto flex w-full max-w-[1400px] flex-col relative pointer-events-auto">
        <div className="relative flex flex-col items-center justify-between gap-4 lg:flex-row lg:gap-0">
          <div className="z-20 flex flex-wrap items-center justify-center gap-2 sm:gap-3 lg:justify-start">
            {leftItems.map((item, idx) => (
              ...
              >
                {item.label}
                {hoveredIndex === idx && (
                  <motion.div layoutId="navbar-indicator-left" className="absolute inset-0 border-b-[3px] border-black" transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                )}
              </a>
            ))}
          </div>

          <div className="pointer-events-auto group z-10 mt-4 flex cursor-pointer items-center justify-center lg:absolute lg:left-1/2 lg:top-1/2 lg:mt-0 lg:-translate-x-1/2 lg:-translate-y-1/2">
          ...
          <div className="z-20 mt-4 flex w-full flex-wrap items-center justify-center gap-2 sm:gap-3 lg:mt-0 lg:w-auto lg:justify-end">
            ...
            <button onClick={onCtaClick} className="flex items-center gap-2 rounded-full bg-[#00ffc6] px-5 py-2.5 text-xs font-black text-black shadow-[0_0_20px_rgba(0,255,198,0.4)] transition-colors hover:bg-[#00e5b2] sm:px-7 sm:text-sm">
              {ctaText}
        ...
        <motion.div style={{ opacity: lineOpacity }} className="mt-6 w-full border-b-2 border-dotted border-white/30" />
      </div>
    </nav>
  );
}
...
export function RobotHero({
  ...
  return (
    <section ref={containerRef} className="relative h-dvh min-h-[600px] w-full overflow-hidden" style={{ background: `linear-gradient(to bottom, ${entorno.fondoArriba} 0%, ${entorno.fondoArriba} 55%, ${entorno.fondoMedio} 65%, ${entorno.fondoAbajo} 100%)` }}>
      <div className="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden" style={{ zIndex: 0 }}>
        <h1 className="select-none whitespace-nowrap font-sans font-black" style={{ color: "#000000", opacity: 0.13, letterSpacing: "-0.05em", fontSize: "clamp(4rem, 15vw, 14rem)", lineHeight: 1, transform: `translate(0px, 40px) rotate(0deg)` }}>
          {backgroundText}
        </h1>
      </div>
      ...
      <div className="absolute inset-0 z-10">
        <Canvas shadows camera={{ position: [0, 0.2, 6], fov: 40 }}>
          ...
          <ResponsiveGroup>
            ...
            <RobotPrototype neckParams={{ baseR: 0.215, baseH: -0.05, midR: 0.28, midH: 0.02, lipBottomR: 0.295, lipBottomH: 0.045, lipTopR: 0.27, lipTopH: 0.055, innerR: 0.1, innerDropH: 0.0 }} bodyParams={{ bodyBevelR: 0.235, bodyBevelY: 0.34, bodyBevelT: 0.025 }} />
          </ResponsiveGroup>
        </Canvas>
      </div>
